...
export const TripsListMulti = ({ trips, onEditTrip, onDeleteTrip, onViewTrip }: TripsListProps) => {
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Regex: allow letters, numbers, spaces, commas, dots, dashes
    // You can customize this pattern as needed
    const allowedPattern = /^[a-zA-Z0-9 ,.-]*$/;
  
    // Filter out disallowed characters by removing them
    if (!allowedPattern.test(value)) {
      // Remove all invalid chars by replacing with ''
 ...
 * 
 * 
 *   <button onClick={() => onViewTrip(trip)} className="bg-gray-300 text-black px-3 py-1 rounded">View</button>
 * 
 * 
 * 
 */}
